...
def taskrange(imax, imin=0, shift=0):
    ...
    if ntask <= 0:
        ...
        print
        "rank: %d size: %d ntask: %d start: %d end: %d" % (rank, size, ntask, start, end - 1)
...
def transfer_data(data, tag, dest=0, mode='append'):
    ...
    if rank != dest:
        print
        "rank%d is sending data to rank%d with tag%d" % (rank, dest, tag)
        comm.send(data, dest, tag=tag)
    else:
        for sender in senders:
            print
            "rank%d is receiving tag%d data from rank%d" % (dest, tag, sender)
            if type(data) == dict:
